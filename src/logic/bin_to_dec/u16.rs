...
impl ToDec<Vec<u16>> for BinToDec<&str, ExactBits<u16>, LittleEndian, Compact> {
    fn convert(self) -> Vec<u16> {
        self.input
            .as_bytes()
...
impl ToDec<Vec<u16>> for BinToDec<&str, ExactBits<u16>, BigEndian, Compact> {
    fn convert(self) -> Vec<u16> {
        self.input
            .as_bytes()
            .chunks(self.bit_alignment.1 as usize)
...
impl ToDec<Vec<u16>> for BinToDec<&str, StandardBits<u16>, BigEndian, Spaced> {
    fn convert(self) -> Vec<u16> {
        self.input
            .split(' ')
            .filter(|s| !s.is_empty())
...
impl ToDec<Vec<u16>> for BinToDec<&str, ExactBits<u16>, BigEndian, Spaced> {
    fn convert(self) -> Vec<u16> {
        self.input
            .split(' ')
            .filter(|s| !s.is_empty())
...
impl ToDec<Vec<u16>> for BinToDec<&str, VariableBits<u16>, BigEndian, Spaced> {
    fn convert(self) -> Vec<u16> {
        self.input
            .split(' ')
            .filter(|s| !s.is_empty())
...
mod tests {
    ...
    #[test]
    fn bin_to_dec_exact_bits_u16_little_endian_compact() {
        let input: &str = &vec!["1010", "1100"].into_iter().collect::<String>();

        let expected = vec![10, 12];
        let actual: Vec<_> = input.bin_to_dec().u16().exact(4).unwrap().convert();
    ...
    #[test]
    fn bin_to_dec_std_bits_u16_big_endian_compact() {
        let input: &str = &vec!["0100000000000000", "0011000000000000"]
            .into_iter()
            .collect::<String>();
        println!("{}", input);

        let expected = vec![2, 12];
        let actual: Vec<_> = input.bin_to_dec().u16().big_endian().convert();

        assert_eq!(expected, actual);
    }

    #[test]
    fn bin_to_dec_exact_bits_u16_big_endian_compact() {
    ...
    #[test]
    fn bin_to_dec_std_bits_u16_big_endian_spaced() {
        let input: &str = &vec!["0100000000000000", "0011000000000000"].join(" ");

        let expected = vec![2, 12];
        let actual: Vec<_> = input.bin_to_dec().u16().big_endian().spaced().convert();

        assert_eq!(expected, actual);
    }

    #[test]
    fn bin_to_dec_exact_bits_u16_big_endian_spaced() {
        let input: &str = &vec!["0100", "0011"].join(" ");
    ...
    #[test]
    fn bin_to_dec_variable_bits_u16_big_endian_spaced() {
        let input: &str = &vec!["0100", "0011"].join(" ");
